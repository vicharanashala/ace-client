...
export default function AudioRecorder({
  ...
  const renderIcon = () => {
    if (isListening === true) {
      return <ListeningIcon className="stroke-green-500" style={{ width: 29, height: 29 }} />;
    }
    if (isLoading === true) {
    ...
    <TooltipAnchor
      ...
      render={
        <button
          ...
          disabled={disabled}
          className={cn(
            'relative flex size-10 items-center justify-center rounded-full p-1 transition-colors hover:bg-surface-hover',
          )}
